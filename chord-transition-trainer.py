...
class MainWindow(QWidget):
    ...
    def __init__(self, app: QApplication,
        ...
        # The following work-around only became neccesary when running
        # binary blobs produced by github actions workflow.
        if chord_list and chord_list[0] == "chord-transition-trainer":
            chord_list = [""]
        self.chord_list = chord_list[:] if chord_list else [""]
        self.tick_path = tick_path
        self.setWindowTitle("Chord Transition Trainer")
        self._set_defaults()
    ...
    def _update_transition_text(self):
        self.chord_transition_text.setText(
            self.TRANSITION_DISPLAY_STR.format(self.current, self.next) )
    ...
    def _ensure_width_for_text(self):
        ...
        needed_width = 100 + metrics.horizontalAdvance(
                                 self.TRANSITION_DISPLAY_STR.format(top_two[0], top_two[1]) )
...
def main(argv):
    ...
    w = MainWindow( app, app_name, tick_path, argv[1:] )
